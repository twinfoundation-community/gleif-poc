...
import { keriTimestamp } from "@gleif/verifier-core";
...
async function getContainerRuntime(): Promise<{
  docker: string;
...
async function issueQviCredential(
  ...
  const credResult = await gleifClient
    .credentials()
    .issue(gleifAidName, credData);
...
async function admitCredential(
  ...
  for (let retry = 0; retry < maxRetries; retry++) {
    ...
    log(
      "IPEX",
      `No grant notification yet, waiting... (${retry + 1}/${maxRetries})`,
    );
  ...
  if (!grantNote) {
    log(
      "IPEX",
      "No pending grant notification found - credential may already be admitted",
    );
    ...
    if (credentials.length > 0) {
      log(
        "IPEX",
        `${entityName} already has ${credentials.length} credential(s)`,
      );
    } else {
      log(
        "IPEX",
        `Warning: ${entityName} does not appear to have the credential`,
      );
    }
    return;
  }
  ...
  if (expectedCredSaid) {
    log(
      "IPEX",
      `Waiting for credential ${expectedCredSaid} to be available...`,
    );
    for (let i = 0; i < maxRetries; i++) {
      ...
      if (creds.length > 0) {
        log(
          "IPEX",
          `${entityName} credential admitted successfully: ${creds[0].sad.d}`,
        );
        return;
      }
      await new Promise((resolve) => setTimeout(resolve, 1000));
      log(
        "IPEX",
        `Credential not yet available, waiting... (${i + 1}/${maxRetries})`,
      );
    }
    ...
    log(
      "IPEX",
      `Warning: ${entityName} credential may not have been admitted properly`,
    );
  } else {
 ...
 */
async function createIotaDidViaBackend(): Promise<{
  did: string;
  document: any;
} | null> {
  log(
    "IOTA",
    `Attempting to create IOTA DID via ${BACKEND_URL}/api/iota/create-did`,
  );
...
async function issueDesignatedAliasesCredential(
  ...
  if (existing) {
    log(
      "CREDENTIAL",
      `Designated Aliases credential already exists: ${existing.sad.d}`,
    );
    return { said: existing.sad.d, credential: existing };
  }
  ...
  log(
    "CREDENTIAL",
    "Issuing Designated Aliases credential (self-signed by LE)...",
  );
  ...
  const credentials = allCredentials.filter(
    (c: any) =>
      c.sad.s === DESIGNATED_ALIASES_SCHEMA_SAID && c.sad.i === lePrefix,
  );
  ...
  log(
    "CREDENTIAL",
    `Designated Aliases credential issued: ${credential.sad.d}`,
  );
...
async function main() {
  ...
  log(
    "PASSCODE",
    `GLEIF passcode generated (first 4 chars): ${gleifPasscode.substring(0, 4)}...`,
  );
  log(
    "PASSCODE",
    `QVI passcode generated (first 4 chars): ${qviPasscode.substring(0, 4)}...`,
  );
  log(
    "PASSCODE",
    `LE passcode generated (first 4 chars): ${lePasscode.substring(0, 4)}...`,
  );
  ...
  const gleifAid = await getOrCreateAid(
    gleifClient,
    GLEIF_AID_NAME,
    WITNESS_AID,
  );
  ...
  const { said: qviCredSaid, credential: qviCredential } =
    await issueQviCredential(
      gleifClient,
      GLEIF_AID_NAME,
      gleifAid.prefix,
      gleifRegistryId,
  ...
  const { said: leCredSaid, credential: leCredential } =
    await issueLeCredential(
      qviClient,
      QVI_AID_NAME,
      qviAid.prefix,
      qviRegistryId,
  ...
  await resolveOobi(
    leClient,
    DESIGNATED_ALIASES_SCHEMA_OOBI,
    "designated-aliases-schema",
  );
  ...
  if (!backendResult) {
    throw new Error(
      "Cannot create Designated Aliases credential: backend unreachable. Run with ./start.sh --with-backend first.",
    );
  }

  const linkedIotaDid = backendResult.did;
  ...
  if (reverseLinked) {
    log(
      "CREDENTIAL",
      `Reverse link established: ${linkedIotaDid} -> ${didWebs}`,
    );
  } else {
    log(
      "CREDENTIAL",
      "Warning: Could not establish reverse link for Direction 2",
    );
  }
  ...
  try {
    ...
    log(
      "CREDENTIAL",
      `Designated Aliases credential issued: ${designatedAliasesCredSaid}`,
    );
  } catch (err: any) {
    log(
      "CREDENTIAL",
      `Warning: Could not issue Designated Aliases credential: ${err.message || err}`,
    );
  ...
  try {
    ...
    log(
      "SALLY",
      `Using container runtime: ${docker}, container: ${containerName}`,
    );
    ...
    const { stdout, stderr } = await execInSallyContainer(
      docker,
      containerName,
      kliCommand,
    );
    ...
    log(
      "SALLY",
      `Warning: Could not configure Sally trust anchor: ${err.message || err}`,
    );
    log("SALLY", "Manual configuration required:");
    log(
      "SALLY",
      `  docker exec direct-sally kli oobi resolve --name ${SALLY_KS_NAME} --passcode ${SALLY_PASSCODE} --oobi "${gleifAid.oobi}"`,
    );
  }
  ...
  if (sallyConfigured) {
    try {
      ...
      log(
        "SALLY",
        "Sally can now verify LE credentials (chain: LE -> QVI -> GLEIF)",
      );
      sallyQviCredPreloaded = true;
    } catch (err: any) {
      log(
        "SALLY",
        `Warning: Could not present QVI credential to Sally: ${err.message || err}`,
      );
      log(
        "SALLY",
        "Sally may need manual credential presentation for chain verification",
      );
    }
  } else {
    log(
      "SALLY",
      "Skipping QVI credential presentation - Sally OOBI not resolved",
    );
    log(
      "SALLY",
      "Manual presentation will be required after Sally is accessible",
    );
  }
  ...
  console.log(
    `  GLEIF (Root) --> QVI Credential --> QVI --> LE Credential --> Legal Entity`,
  );
  console.log();
  ...
  if (designatedAliasesCredSaid) {
    console.log(
      `  DA Cred:     ${designatedAliasesCredSaid} (Designated Aliases)`,
    );
  ...
  console.log(
    `  ${sallyConfigured ? "[OK]" : "[!]"} GLEIF resolved Sally OOBI`,
  );
  console.log(
    `  ${sallyGleifOobiResolved ? "[OK]" : "[!]"} Sally configured with GLEIF trust anchor`,
  );
  console.log(
    `  ${sallyQviCredPreloaded ? "[OK]" : "[!]"} QVI credential pre-loaded into Sally`,
  );
  if (sallyConfigured && sallyGleifOobiResolved && sallyQviCredPreloaded) {
    ...
    if (!sallyQviCredPreloaded) {
      console.log(
        "    - Present QVI credential to Sally (via IPEX grant from GLEIF to Sally)",
      );
    }
  }
  console.log();
